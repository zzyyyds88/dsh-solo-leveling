...
import { readStoredNumber, writeStoredNumber } from "../persist.js";
/**
 ...
 */
export function useResizableSplit(options = {}) {
    const { defaultWidth = 50, minWidth = 20, maxWidth = 80, storageKey, unit = 'ratio', } = options;
    const isPx = unit === 'px';
    const [width, setWidthState] = useState(() => storageKey === undefined
        ? defaultWidth
    ...
    const handleProps = { onPointerDown: handlePointerDown, onDoubleClick: handleDoubleClick };
