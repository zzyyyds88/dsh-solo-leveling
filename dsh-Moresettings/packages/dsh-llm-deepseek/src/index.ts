 ...
 */
export function resolveAdapterOptions(config: Config, environment?: LaunchEnvironmentSnapshot, defaults?: DeepSeekDefaults): ResolvedDeepSeekOptions {
  ...
  const fallbackRetry = Number.isSafeInteger(defaultsRetry) && (defaultsRetry as number) >= 0
    ? { mode: 'normal' as const, maxRetries: defaultsRetry as number }
    : undefined
  return {
...
export function apply(ctx: Context, config: Config): void {
  ...
  // (the commit event, fired after the resolved value is swapped) rather than
  // `settings/document-updated` (fires from bumpRevision before the swap).
  ctx.effect(() => {
