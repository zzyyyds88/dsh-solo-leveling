...
function resolveModelReasoning(
  ...
  if (efforts === undefined) {
    // Local fork: a hand-declared OpenAI-style model gets the default
    // off/low/medium/high levels; a catalog-backed model keeps its own
    // metadata; other protocols stay non-reasoning unless declared.
...
function resolveModelCompat(
  ...
  if (thinkingFormat === undefined && supportsReasoningEffort === undefined) {
    // Local fork: a hand-declared model on OpenAI-completions defaults to
    // supporting reasoning effort, matching the reasoning default above.
    if (base === undefined && api === 'openai-completions') return { compat: { supportsReasoningEffort: true } }
    return {}
  }
