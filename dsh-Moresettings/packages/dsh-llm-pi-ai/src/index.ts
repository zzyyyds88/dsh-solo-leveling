...
export type { PiAiDefaults } from './config.ts'
...
export function apply(ctx: Context, config: Config): void {
  ...
  /**
   * The `dsh-defaults` namespace section consumed by profile resolution
   * (Local fork: the retry-count default). `settings.get` returns the frozen
   * resolved value, so identity tracks every namespace change.
   */
  const defaultsSnapshot = (): PiAiDefaults | undefined => {
    const value = settings?.get('dsh-defaults')
    return value === undefined ? undefined : value as PiAiDefaults
  }
  /**
  ...
  // to `settings/updated` (the commit event) rather than
  // `settings/document-updated`: the latter fires from bumpRevision BEFORE
  // the registration's resolved value is replaced, so a listener would read
  // the stale value; the former fires after the swap, per commit.
  ctx.effect(() => {
