...
/**
 * Build-time injected start directory (local fork customization). Injected by
 * the build config from the DSH_PICKER_DEFAULT_PATH environment variable;
 * empty/unset keeps the official behavior (open at the host home directory).
 */
declare const DSH_PICKER_DEFAULT_PATH: string | undefined

/** The directory the picker opens at; undefined defers to the host home directory. */
const PICKER_START_PATH: string | undefined =
  typeof DSH_PICKER_DEFAULT_PATH === 'string' && DSH_PICKER_DEFAULT_PATH.length > 0
    ? DSH_PICKER_DEFAULT_PATH
    : undefined
...
export function DirectoryBrowser({ open, listDirectory, createDirectory, onOpen, onClose, busy, t }: DirectoryBrowserProps) {
  ...
  // Every open starts fresh at the start directory (PICKER_START_PATH when
  // configured, otherwise the host home directory); closing invalidates any
