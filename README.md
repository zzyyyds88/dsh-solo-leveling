...
| `思考强度与重试默认值/` | 第三方模型可在对话框直接调思考强度（默认 off/low/medium/high，可精确声明）；默认重试次数 2→5 | 已实施，升级后需重打（后端需重启生效） |
| `全网监听与登录鉴权/` | caddy HTTPS 反代（`https://<IP>:5700`）+ 口令登录门闸（首次 `/setup` 设口令、设置面板改口令、会话 Cookie + 限速）；dsh 仅监听回环 | 已实施（含 HTTPS 切换脚本），升级后需重打 3 处安装包补丁 + 重跑 switch-to-https.sh |
| `定制插件化改造/` | 把上面三个补丁项目改造成**源码构建 + profile 挂载的正式插件**（同包名覆盖：webserver 门闸 / apiproxy 命名空间 / connection 放行 / 目录选择器默认路径 / pi-ai 思考强度 + retryPolicy 配置化），升级免疫 | 五个 fork 构建 + 测试环境全链路验证通过；测试环境已迁移至工作区级 `test-env/`；正式安装待用户执行 `install-to-profile.sh` |
| `test-env/` | **工作区级专用 DSH 测试环境**：独立 DSH_HOME + 端口 3090，打包测试唯一去处（脚本：`scripts/test-env-*.sh`） | 已部署（由已验证的项目级 test-env 复制，含五个 fork） |
