...
| 项目 | 一句话说明 | 状态 |
|---|---|---|
| `修改默认工作目录/` | Web GUI 目录选择器默认打开 `/home/user/Projects`（替代 `/root`） | 已实施，升级后需重打；**已由 `dsh-defaults/` 统一插件替代**（默认目录改设置页配置） |
| `思考强度与重试默认值/` | 第三方模型可在对话框直接调思考强度（默认 off/low/medium/high，可精确声明）；默认重试次数 2→5 | 已实施，升级后需重打；**已由 `dsh-defaults/` 统一插件替代**（思考强度保持内建默认、重试次数改设置页配置） |
...
| `dsh-defaults/` | **统一默认值插件**（合并「修改默认工作目录」+「思考强度与重试默认值」）：设置 → 插件 → 「默认值」标签页配置默认工作目录与默认重试次数，改设置即时生效无需重启 | 源码即产物 + 三个 fork；test-env 验证通过（`verify-defaults.mjs` 9 项断言）；正式安装待用户执行 `install-to-profile.sh` |
