...
> [README.md](README.md) 与 [docs/整合迁移路线图.md](docs/整合迁移路线图.md)。
...
8. **一切可调参数必须进「设置 → 插件 → 插件配置」卡片，禁止固化。** 整合包内今后
   ...
   「设置 → 插件 → 插件配置」区的独立卡片（`settings.plugin.item`，样式同官方
   「网页搜索」卡片）暴露出来、即时生效，**绝不允许把参数硬编码进源码/样式/资源**。
