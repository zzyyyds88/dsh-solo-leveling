...
| `dsh-client-ui-mobile-adapt` | client | **手机端适配**：窄屏聊天区占满全宽、右侧面板变抽屉（含遮罩关闭）、输入框 16px 防 iOS 缩放、虚拟键盘避让（输入卡可滚到键盘上方、桌宠让位）、桌宠窄屏默认展开并抬到输入区上方（缩放可调，最小化仍贴角）、会话标题行窄屏水平居中；「移动端适配」设置卡可调总开关/断点/三个抽屉宽度/桌宠缩放（保存即生效） |
