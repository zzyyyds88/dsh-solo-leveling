...
| `ctx.fileReferences` | `seam` | [`file-reference`](../packages/context/file-reference) | [`file-reference-local`](../packages/context/file-reference-local) | - | - | 该接口通过其一元 Remote 契约，在被寻址的 Agent cwd 内返回仅含路径的补全候选；提供方负责命名空间访问与排序，且不读取文件内容。 |
