...
`SessionReferenceMentionCandidate` extends that record with the canonical prompt mention; `FileReferenceCandidate` is the path-only completion record for the file-reference seam.
