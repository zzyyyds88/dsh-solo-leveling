...
| [task-board.md](task-board.md) | the task-board seam: `TaskRecord` ledger rows, cron `ScheduleRule`s, execution records, and the `ctx.taskBoard` service with its `task-board/changed` full-snapshot event |
