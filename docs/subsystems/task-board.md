...
The task board owns the DSH kanban ledger as a durable host-side snapshot: five-column task records with cron schedules and execution history, persisted to `$DSH_HOME/task-board/ledger.json` and mutated only by the `ctx.taskBoard` service inside the host process ([`@deepseek-ai/dsh-host-task-board`](../../packages/host/task-board)). The browser board ([`@deepseek-ai/dsh-client-ui-task-board`](../../packages/client/ui-task-board)) and the mobile remote are pure views over the same `/api/task-board/*` routes (SSE full snapshots); neither holds timers nor ledger state. The [package](../../packages/host/task-board) owns composition, execution settlement, and route behavior; this page records the service and event surface that other host plugins consume.
