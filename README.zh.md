...
| **手机遥控** | DSH 遥控器 App（安卓瘦客户端）与本机 dsh 之间的 BFF 插件：`/api/mobile/*` 握手、白名单内 RPC 进程内转发、审批/问答应答包装、工作区圈根的产物读取，外加 `/api/mobile/events` 单向下行事件流（mux/host/taskboard），鉴权复用访问门禁；总开关与请求体上限在设置卡，保存即生效 |
