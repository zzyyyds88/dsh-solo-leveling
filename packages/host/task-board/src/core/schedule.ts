...
export function parseCron(expr: string): CronSchedule | null {
  ...
  for (let index = 0; index < 5; index++) {
    const [min, max] = FIELD_RANGES[index] ?? [0, 0]
    const set = new Set<number>()
    if (!parseField(fields[index] ?? '', min, max, set)) return null
    sets.push(set)
  }
  const weekdays = new Set<number>()
  for (const day of sets[4] ?? new Set<number>()) weekdays.add(day === 7 ? 0 : day)
  return {
    minutes: sets[0] ?? new Set<number>(),
    hours: sets[1] ?? new Set<number>(),
    days: sets[2] ?? new Set<number>(),
    months: sets[3] ?? new Set<number>(),
    weekdays,
