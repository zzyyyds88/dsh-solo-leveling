...
| [`access-gate/`](access-gate/README.md) | Login gate: `/setup` first-run password, session cookie, rate limiting | registers `access-gate` settings namespace |
| [`git-graph/`](git-graph/README.md) | Workspace-gated git service and `/git/*` routes for the browser graph | consumes `ctx.webServer`, `ctx.workspaceRegistry`, `ctx.subprocess` |
| [`aionui-panel/`](aionui-panel/README.md) | Workspace-gated fs + git services and `/aionui-panel/*` routes for the right panel | consumes `ctx.webServer`, `ctx.workspaceRegistry`, `ctx.subprocess`, `ctx.systemPrompt` |
