...
export function createApiProxy(ctx: Context, defaults: ApiProxyDefaults): ApiProxy {
  ...
  /** Whether this deployment can hand a text document to a native editor. (Local fork: settings open guard.) */
  ...
  return {
    ...
    settings: {
      ...
      async openDocument(request, signal) {
        ...
        // Local fork: headless deployments without a native editor report
        // opened:false instead of failing the open request.
        ...
        const disposers = [
          ...
          ctx.on('agent/error', ({ agent, error }: { agent: Agent; error: unknown }) => {
            queue.push(frame({
              ...
              sessionId: agent.id,
              // Local fork: redact AUTH before it leaves the host: provider
              // messages may echo the credential, and this relay is the
              // pet/live surface's source.
              message: (error as { code?: unknown } | null)?.code === 'AUTH'
                ? 'API key is invalid'
                : errorChain(error),
            }))
          }),
