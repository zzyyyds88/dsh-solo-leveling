...
export function createExplorerStore(api: PanelApi): ExplorerStore {
  ...
  const ensureDir = async (root: string, rel: string): Promise<void> => {
    ...
    handle.update((prev) => {
      ...
      // oxlint-disable-next-line no-dynamic-delete -- rel is a runtime dir key
  ...
  const store: PreviewStore = Object.assign(handle, {
    setRoot(root: string) {
      handle.update((prev) => {
        ...
        const activeTabId = tabs.length > 0 ? (tabs[tabs.length - 1]?.id ?? null) : null
    ...
    closeTabs(ids: string[]) {
      ...
      const activeTabId = active
        ? state.activeTabId
        : remaining.length > 0
          ? remaining[Math.min(state.tabs.findIndex(tab => tab.id === state.activeTabId), remaining.length - 1)]?.id
    ...
    async handleFsChange() {
      ...
      if (active === undefined || active.content === null || active.dirty
        || active.diff !== undefined || !isTextType(active.contentType)) return
