...
describe('ui-theme apply', () => {
  ...
  it('serves remote browsers from Host settings too (Local fork: docs/工作区/升级适配指南.md §2.1)', async () => {
    // The fork binds host persistence on every browser: a remote boot reads
    // the document and a theme change writes through instead of staying
    // process-local. A previous revision asserted the official memory-mode
    // contract here — restoring it would silently reintroduce that revert.
