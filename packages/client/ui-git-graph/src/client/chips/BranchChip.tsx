...
/** Full props of the branch chip: either seat's runtime share (the session-maybe context hole or the
 * session-scoped dock fallback) + the git-graph inject face + the locale seat. */
...
export function BranchChip(props: BranchChipProps) {
  ...
  const heroSeat = sessionSnapshot?.composerPhase === 'blank' && (sessionSnapshot.openState === 'open' ||  blankSession)
  ...
  useEffect(() => {
    ...
    void props.branches(sessionId).then((view) => { if (live) setBranchesView(view) })
    return () => { live = false }
