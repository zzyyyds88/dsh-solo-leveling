 ...
 * @module @deepseek-ai/dsh-client-ui-skin-center/skin-switch
 */

import { chmodSync, lstatSync, mkdirSync, mkdtempSync, readdirSync, readFileSync, realpathSync, renameSync, rmdirSync, rmSync, statSync, symlinkSync, unlinkSync, writeFileSync } from 'node:fs'
import { homedir } from 'node:os'
import { basename, dirname, join as joinPath, resolve as resolvePath, sep } from 'node:path'
import { fileURLToPath } from 'node:url'

/**
 * Walk up from a file location to the nearest @deepseek-ai/ scoped dir
...
export function findScopedAnchor(fromDir: string): string | null {
  let current = fromDir
  for (;;) {
    const scoped = joinPath(current, '@deepseek-ai')
    try {
 ...
 *    (packages/skins/ or node_modules/@deepseek-ai/);
 *  - pnpm virtual-store layout: the nearest @deepseek-ai/ scoped dir found by
...
export function resolveSkinsDir(fromUrl: string = import.meta.url): string {
  ...
  return candidates[0] ?? dirname(here)
}
 ...
 *    and per-skin npm packages @deepseek-ai/dsh-client-ui-skin-<id>);
 *  - the bundled-skins carrier: @deepseek-ai/dsh-skins/skins/<id> (skin assets
...
export function listSkinDirCandidates(skinsDir: string): string[] {
  ...
  for (const dir of entries) {
    ...
    // node_modules/@deepseek-ai/dsh-skins -> real dir) would otherwise be
...
export function loadRegistry(skinsDir: string = SKINS_DIR): Record<string, SkinSwitchEntry> {
  ...
  for (const dir of listSkinDirCandidates(skinsDir)) {
    ...
    if (existing !== undefined) {
      ...
      console.warn('[skin-center] duplicate skin id "' + meta.id + '": keeping ' + existing.dir + ', ignoring ' + dir)
      continue
    }
    out[meta.id] = {
      pkg: meta.package,
      id: meta.wiring.id,
      dir,
      bundleWired:  meta.wiring.bundleWired,
    }
  }
  return out
}
...
export function renderManaged(active: string | null, registry: Record<string, SkinSwitchEntry> = loadRegistry()): string {
  ...
  for (const [name, entry] of Object.entries(registry)) {
    ...
    lines.push(`- id: ${entry.id}`, '  disabled: true')
  }
  if (active !== null && !wired.has(active)) {
    const activeEntry = registry[active]
    if (activeEntry !== undefined) {
      lines.push('- insert:', `    - id: ${activeEntry.id}`, `      name: ${yamlSingleQuote(activeEntry.pkg)}`)
    }
  }
 ...
 * layout where the skin package sits at node_modules/@deepseek-ai/..., or a
...
export function useSkin(name: string, opts: { home?: string; profile?: string; registry?: Record<string, SkinSwitchEntry> } = {}): string {
  ...
  if (!official) {
    const entry = registry[name] as SkinSwitchEntry
...
export function currentSkin(
  ...
  opts: { home?: string; profile?: string; registry?: Record<string, SkinSwitchEntry> } = {},
): string {
  const paths = resolvePaths(opts.home, opts.profile)
  const registry = opts.registry ?? loadRegistry()
