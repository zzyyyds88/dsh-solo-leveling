 ...
 * mobile-adaptation parameters (master switch / breakpoint / drawer
 * widths / pet scale).
 */
...
export const zh = {
  ...
  'settings.enabled': '启用移动端适配',
  'settings.enabledHint': '关闭后窄屏恢复桌面布局（抽屉/键盘避让等全部停用）。',
  'settings.breakpoint': '窄屏断点 (px)',
  'settings.breakpointHint': '视口宽度 ≤ 此值时启用移动端布局（侧栏/详情/右侧面板变抽屉）。支持 320–1280。',
  'settings.sidebarWidth': '侧栏抽屉宽度 (px)',
  'settings.sidebarWidthHint': '左侧会话侧栏抽屉的宽度上限。支持 200–500。',
  'settings.detailsWidth': '详情抽屉宽度 (px)',
  'settings.detailsWidthHint': '右侧详情面板抽屉的宽度上限。支持 280–640。',
  'settings.drawerWidth': '面板抽屉宽度 (px)',
...
export const en = {
  ...
  'settings.description': 'Configure the narrow-screen breakpoint, drawer widths, and pet scale. Saving takes effect immediately — no restart needed.',
  'settings.enabled': 'Enable mobile adaptation',
  'settings.enabledHint': 'Off restores the desktop layout on narrow screens (drawers, keyboard avoidance, and other overrides all stop).',
  ...
  'settings.sidebarWidth': 'Sidebar drawer width (px)',
  'settings.sidebarWidthHint': 'Width cap of the left session-sidebar drawer. 200–500.',
  'settings.detailsWidth': 'Details drawer width (px)',
  'settings.detailsWidthHint': 'Width cap of the right details-panel drawer. 280–640.',
  'settings.drawerWidth': 'Panel drawer width (px)',
