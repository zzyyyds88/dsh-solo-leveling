...
export interface PetSnapshot {
  ...
  partial?: { blocks?: PetNode[] }
  running?: boolean
  queue?: PetNode[]
  nodes?: PetNode[]
}

/** One derived pet state. */
...
function recentToolFailure(snapshot: PetSnapshot | null | undefined): string {
  const nodes = Array.isArray(snapshot?.nodes) ? snapshot.nodes : []
...
function state(kind: string, label: string, detail: string, extra: Record<string, unknown> = {}): PetState {
