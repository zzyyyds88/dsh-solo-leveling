...
> **本地维护（已脱离上游）**：本皮肤已并入本仓库插件集（`@zzyyyds88/dsh-skins`），作为插件家族的一部分随插件一起分发/升级，**不再跟随上游 `dsh-external/dsh-deep-whale` 更新**。源码可自由解耦与改造（许可 CC BY-NC-SA 4.0 不变，署名链见 NOTICE）。
...
- **插件专用适配**：皮肤重映射了 260+ 个 `--dsw-*` token，另对不走 `--dsw-*` 的桌宠（`--pet-blue/--pet-cyan` → periwinkle/柔金）与右侧面板（`--aion-*` token → 皮肤色板）做了专门覆盖，使其随皮肤一起变化
...
None, as the plugin is a browser-side skin asset.

#### KV Cache effect

The skin ships only CSS and theme tokens applied in the browser; it registers nothing model-facing and never touches a session prefix.

## Known Limitations and Deferred Work

- The skin is distributed as a prebuilt client bundle with embedded data-URI assets; changing any art requires a rebuild in the scaffold repo and committing the regenerated `lib/` back here.
- The loading/thinking/tool-state animation hooks are reserved stable slots, not yet driven by real runtime events.
- The skin is released under CC BY-NC-SA 4.0, so any commercial use is prohibited.
