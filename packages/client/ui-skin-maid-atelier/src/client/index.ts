...
export function apply(ctx: Context): void {
  ...
  // oxlint-disable-next-line prefer-const -- declared here, assigned later for closures
  ...
  // oxlint-disable-next-line no-non-null-assertion -- widthSheet is a fresh <style> element; its sheet is non-null after append
  ...
  const appendRule = (rule: string): void => {
    // oxlint-disable-next-line no-non-null-assertion -- fresh style element sheet
  ...
  recoverRailSearchFocus = (event: MouseEvent): void => {
    ...
    const recover = (): void => {
      ...
      const searchRoot = input?.closest<HTMLElement>("[class*='search']") ?? null
      if (input !== null && searchRoot !== null) {
        searchRoot.click()
  ...
  observer = new MutationObserver((records) => {
    ...
    for (const record of records) {
      ...
      if (appNodes.length > 0 && (appNodes.some(node => nodeTouches(node, PROJECTED_STATE_SELECTOR))
        ||  target.matches("header, [data-slot='sidebar.settings']"))) {
        projectedStateChanged = true
      }
    }
