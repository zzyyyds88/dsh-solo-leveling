...
export const SERVICE_API: readonly ServiceApiEntry[] = [
  ...
  {
    key: 'taskBoard',
    summary: 'The board\'s single writer: ledger state, persistence, scheduling, and executions (see module doc).',
    description: 'The board\'s single writer: ledger state, persistence, scheduling, and executions (see module doc).',
    methods: [
      {
        signature: 'async start(): Promise<void>',
        description: 'Load the ledger, then arm everything that reads it: routes may already be registered (they serve whatever is loaded), so the scheduler and frame watcher only start after recovery completes.',
        parameters: [],
      },
      {
        signature: 'stop(): void',
        description: 'Stop ticking, abort the frame watch, and refuse further runs (idempotent).',
        parameters: [],
      },
      {
        signature: 'async flushPersistence(): Promise<void>',
        description: 'Resolve once every queued ledger write reached disk (tests and graceful shutdown use it; ordinary callers rely on the chain\'s ordering alone).',
        parameters: [],
      },
      {
        signature: 'listTasks(): readonly TaskRecord[]',
        description: 'The current in-memory ledger (the SSE/route snapshot source).',
        parameters: [],
        returns: 'the task records in ledger order.',
      },
      {
        signature: 'createTask(body: unknown): TaskRecord',
        description: 'Append a task. Accepts either minimal create fields (`NewTaskInput`) or a fuller row (id/createdAt/executions included) — the client diff-sync and the migrate path send rows they already hold, and honoring their identity keeps their ids stable across the move onto the host.',
        parameters: [{ name: 'body', description: 'the JSON request body (task fields at the top level).' }],
        returns: 'the stored record.',
        throws: ['BoardError bad-request on invalid fields or a blank title; conflict when a provided id already exists.'],
      },
      {
        signature: 'updateTask(id: string, patch: unknown): TaskRecord',
        description: 'Patch one task\'s editable fields. Host-owned fields (executions, ids, timestamps beyond updatedAt) are ignored in the patch: the ledger copy a client diffed against may be stale, and overwriting execution history or a live status with it would lose real runs.',
        parameters: [{ name: 'id', description: 'the task to update.' }, { name: 'patch', description: 'editable-field changes (explicit undefined clears a pin).' }],
        returns: 'the updated record.',
        throws: ['BoardError not-found on an unknown id; bad-request on invalid values.'],
      },
      {
        signature: 'deleteTask(id: string): void',
        description: 'Remove one task.',
        parameters: [{ name: 'id', description: 'the task to remove.' }],
        throws: ['BoardError not-found on an unknown id.'],
      },
      {
        signature: 'importLedger(body: unknown): number',
        description: 'Import a client ledger exactly once: accepted only while the host ledger is empty (the idempotence guard for two tabs racing their migration); otherwise the batch is discarded and `{imported: 0}` reports the no-op. Invalid rows are dropped by the shared normalizer.',
        parameters: [{ name: 'body', description: 'the JSON request body (`{tasks: unknown[]}`).' }],
        returns: 'how many rows were imported.',
        throws: ['BoardError bad-request when `tasks` is missing or not an array.'],
      },
      {
        signature: 'async startRun(id: string): Promise<{ executionId: string; sessionId: string }>',
        description: 'Execute a task: open an execution record, flip the card to running, and drive the real session chain in the background. A second call while the task is already running is refused (scheduler + manual-run mutex).',
        parameters: [{ name: 'id', description: 'the task to execute.' }],
        returns: 'the execution/session pair once the session exists.',
        throws: ['BoardError not-found on an unknown task; conflict while the task is already running or when the execution session cannot be created.'],
      },
      {
        signature: 'async runTask(id: string): Promise<boolean>',
        description: 'The scheduler face of startRun: acceptance means the card flipped to running (browser parity — the schedule rolls forward once the trigger is taken); a later launch failure settles the execution record failed instead of keeping the slot, which would re-fire every tick.',
        parameters: [{ name: 'id', description: 'the task to execute.' }],
        returns: 'whether the run was accepted (an unknown task or a held mutex yields false instead of throwing).',
      },
    ],
  },
  {
...
export const EVENT_API: readonly EventApiEntry[] = [
  ...
  },
  {
    ...
    mode: 'emit',
    signature: '\'task-board/changed\'(payload: { tasks: readonly TaskRecord[] }): void',
    summary: 'The task ledger changed; the argument is the new full snapshot.',
    description: 'The task ledger changed; the argument is the new full snapshot. Emitted after every mutation (CRUD, execution records, schedule roll-forward).',
    parameters: [{ name: 'payload', description: '`{tasks}` full-snapshot envelope.' }],
  },
  {
    name: 'tools/change',
    mode: 'emit',
